...
class FusionModel(nn.Module):
    ...
    def forward(
        ...
        # Clean memory before starting block iterations (important for block swap)
        if self.video_model.blocks_to_swap or self.audio_model.blocks_to_swap:
            from ovi.utils.custom_offloading_utils import _clean_memory_on_device
            _clean_memory_on_device(vid.device)
        
        input_device = vid.device
        for i in range(self.num_blocks):
            """
            1 fusion block refers to 1 audio block with 1 video block.
            """
            skip_this_block = (slg_layer > 0 and i == slg_layer)
            
            if not skip_this_block:
                # Block swap logic for video model - wait for block to be ready
                if hasattr(self.video_model, 'offloader') and self.video_model.blocks_to_swap is not None and self.video_model.blocks_to_swap > 0:
                    self.video_model.offloader.wait_for_block(i)

                # Block swap logic for audio model - wait for block to be ready
                if hasattr(self.audio_model, 'offloader') and self.audio_model.blocks_to_swap is not None and self.audio_model.blocks_to_swap > 0:
                    self.audio_model.offloader.wait_for_block(i)
            ...
            # CRITICAL: Always submit block swaps even for skipped blocks!
            # This maintains the circular swap pattern for forward-only mode
            # Block swap logic for video model - submit next swap
            if hasattr(self.video_model, 'offloader') and self.video_model.blocks_to_swap is not None and self.video_model.blocks_to_swap > 0:
                self.video_model.offloader.submit_move_blocks_forward(self.video_model.blocks, i)

            # Block swap logic for audio model - submit next swap
            if hasattr(self.audio_model, 'offloader') and self.audio_model.blocks_to_swap is not None and self.audio_model.blocks_to_swap > 0:
                self.audio_model.offloader.submit_move_blocks_forward(self.audio_model.blocks, i)
        
        # Ensure output is on the correct device (block swap may have moved it)
        if vid.device != input_device:
            vid = vid.to(input_device)
        if audio.device != input_device:
            audio = audio.to(input_device)
